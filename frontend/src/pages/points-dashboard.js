      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p id="pendingPointsDisplay" class="text-xs text-orange-500 font-semibold mt-1"></p>
          </div>
    ...
    <div id="deliveryModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div class="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 space-y-6">
        ...
        <form id="deliveryForm" class="space-y-4">
          ...
          <div>
            <label class="block mb-1.5 text-sm font-semibold text-slate-700">Evidencia fotográfica</label>
            <input
              id="deliveryEvidence"
              type="file"
              accept="image/png, image/jpeg, image/webp"
              class="w-full border rounded-xl p-2.5 text-sm outline-none focus:ring-2 focus:ring-green-600 bg-white"
            >
            <p class="text-xs text-slate-400 mt-1">Foto de tus residuos en el punto de acopio (JPG, PNG o WEBP, máx. 5MB).</p>
          </div>
          ...
          <p class="text-xs text-slate-400 text-center">Tus puntos quedarán pendientes hasta que un administrador valide la entrega.</p>
        </form>
      </div>
    </div>
